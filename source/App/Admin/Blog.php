...
class Blog extends Admin
{
    ...
    public function post(?array $data): void
    {
        ...
        if (!empty($data["upload"]) && !empty($_FILES["image"])) {
            $files = $_FILES["image"];
            $upload = new Upload();
            $image = $upload->image($files, $_FILES["image"]["name"]."-" . time());
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            $postCreate->content = str_replace(["../../../", "{title}"], ["[URL]/", $postCreate->title], $content);
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            $postEdit->content = str_replace(["../../../", "{title}"], ["[URL]/", $postEdit->title], $content);
        ...
        echo $this->view->render("widgets/blog/post", [
            ...
            "authors" => (new User())->find("level >= :level", "level=2")->fetch(true)
        ]);
    }
