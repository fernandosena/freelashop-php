...
abstract class Model
{
    ...
    public function fetch(bool $all = false)
    {
        try {
            ...
            $this->fail = $exception;
            (new Log())->archive()->error("Erro ao trazer todos os resultados", ["exception"=>$exception->getMessage()]);
            return null;
        }
    }
    ...
    protected function create(array $data): ?int
    {
        try {
            ...
            $this->fail = $exception;
            (new Log())->archive()->error("Erro no insert", ["exception"=>$exception->getMessage()]);
            return null;
        }
    }
    ...
    protected function update(array $data, string $terms, string $params): ?int
    {
        try {
            ...
            $this->fail = $exception;
            (new Log())->archive()->error("Erro no update", ["exception"=>$exception->getMessage()]);
            return null;
        }
    }

    /**
     * Regra padrão de salvamento de dados
     * @return bool
     */
    public function save(): bool
    {
        //Verifica se os dados que são obrigadtorios foram informados
        if (!$this->required()) {
            $this->message->warning("Preencha todos os campos para continuar");
            (new Log())->archive()->warning($this->message->getText());
            return false;
        }

        //Atualiza dados se existir um ID
        /** Update */
        if (!empty($this->id)) {
            $id = $this->id;
            ...
            if ($this->fail()) {
                ...
                (new Log())->archive()->error($this->message->getText(), ["exception"=>$this->fail()]);
                return false;
            }
        }

        //Cria um novo dados caso não exista um ID
        /** Create */
        if (empty($this->id)) {
        ...
        //Atualiza o data com as novas informações
        $this->data = $this->findById($id)->data();
        return true;
    }

    /**
     * Retorna o ultimo ID que foi cadastrado
     * @return int
     */
    ...
    public function delete(string $terms, ?string $params): bool
    {
        try {
            ...
            $this->fail = $exception;
            (new Log())->archive()->error("Erro no deletar", ["exception"=>$exception->getMessage()]);
            return false;
        }
    }
